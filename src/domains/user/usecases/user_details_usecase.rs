...
impl UserDetailsUseCase {
    ...
    pub async fn update_user_details(
        ...
        if let Some(ref first) = req.first_name {
            ...
            if let Err(e) = crate::domains::user::validators::user_details_validator::validate_name_part(&req.first_name, "first_name") {
                 if let AppError::ValidationError(_, Some(details)) = e {
                     validation_errors.extend(details);
                 }
            }
        ...
        if let Some(ref last) = req.last_name {
            if let Err(e) = user_validator::validate_no_xss(last, "last_name") {
             ...
             if let Err(e) = crate::domains::user::validators::user_details_validator::validate_name_part(&req.last_name, "last_name") {
                 if let AppError::ValidationError(_, Some(details)) = e {
                     validation_errors.extend(details);
                 }
             }
        ...
        if let Some(ref phone) = req.phone_number {
            ...
            if let Err(e) = crate::domains::user::validators::user_details_validator::validate_phone_number(&req.phone_number) {
                 if let AppError::ValidationError(_, Some(details)) = e {
                     validation_errors.extend(details);
                 }
            }
        ...
        if let Some(ref address) = req.address {
             ...
             if let Err(e) = crate::domains::user::validators::user_details_validator::validate_address(&req.address) {
                 if let AppError::ValidationError(_, Some(details)) = e {
                     validation_errors.extend(details);
                 }
             }

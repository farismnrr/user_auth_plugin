...
use crate::domains::user::dtos::user_details_dto::{UpdateUserDetailsRequest, UserDetailsResponse};
...
impl UserDetailsUseCase {
    ...
    pub async fn update_user_details(
        ...
        if let Some(ref first) = req.first_name {
            ...
            if let Err(e) =
                crate::domains::user::validators::user_details_validator::validate_name_part(
                    &req.first_name,
                    "first_name",
                )
            {
        ...
        if let Some(ref phone) = req.phone_number {
            ...
            if let Err(e) =
                crate::domains::user::validators::user_details_validator::validate_phone_number(
                    &req.phone_number,
                )
            {
                if let AppError::ValidationError(_, Some(details)) = e {
        ...
        if let Some(ref address) = req.address {
            ...
            if let Err(e) =
                crate::domains::user::validators::user_details_validator::validate_address(
                    &req.address,
                )
            {
                if let AppError::ValidationError(_, Some(details)) = e {
        ...
        if !validation_errors.is_empty() {
            let first_msg = validation_errors[0].message.clone();
        ...
        let current_details = self
            ...
            .await?
            .ok_or_else(|| {
                AppError::NotFound(format!("User details not found for user {}", user_id))
            })?;

        // Logic to construct new full_name
        let (current_first, current_last) =
            Self::split_full_name(current_details.full_name.as_deref());
    ...
    pub async fn get_user_details(&self, user_id: Uuid) -> Result<UserDetailsResponse, AppError> {
        let user_details = self
            .repository
            .find_by_user_id(user_id)
            .await?
    ...
    pub async fn update_profile_picture(
        ...
        if let Some(item) = payload.next().await {
            ...
            if file_data.is_empty() {
                return Err(AppError::BadRequest(
                    "Bad Request / Missing file".to_string(),
                ));
            }
            ...
            #[cfg(unix)]
            {
                ...
                std::fs::set_permissions(&filepath, permissions).map_err(|e| {
                    AppError::InternalError(format!("Failed to set file permissions: {}", e))
                })?;
            }
            ...
            log::info!(
                "Profile picture uploaded successfully: {} for user {}",
                profile_picture_url,
                user_id
            );
            let user_details = self
                .repository
                .update_profile_picture(user_id, profile_picture_url)
                .await?;
